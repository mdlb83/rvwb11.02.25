...
// iOS: 4 photos per campground (~331MB)
// Android: Only uses first 2 photos at runtime to meet 200MB Play Store limit
    ...
    photosToBundle.forEach((photo, index) => {
      if (photo.localPath) {
        ...
        if (fs.existsSync(fullPath)) {
          ...
          missingCount++;
          if (missingCount <= 10) {
            console.warn(`⚠️  Asset not found: ${photo.localPath}`);
          }
        }
      }
    });
...
if (missingCount > 10) {
  console.warn(`⚠️  ... and ${missingCount - 10} more assets not found`);
}
...
console.log(`✅ Generated bundled photo mapping with ${photoCount} photos`);
console.log(`📦 Only bundled first ${MAX_BUNDLED_PHOTOS_PER_CAMPGROUND} photos per campground`);
if (skippedCount > 0) {
  console.log(`ℹ️  ${skippedCount} additional photos will be loaded on-demand via API`);
}
if (missingCount > 0) {
  console.log(`⚠️  ${missingCount} assets from JSON not found on disk (skipped)`);
}
